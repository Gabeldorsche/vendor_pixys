...
func init() {
	android.RegisterModuleType("pixys_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pixysExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pixysExpandVariables(ctx, dep_file)
	...
	cmd := pixysExpandVariables(ctx, String(g.properties.Cmd))
